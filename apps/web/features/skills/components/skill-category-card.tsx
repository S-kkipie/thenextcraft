...
export function SkillCategoryCard({ category }: { category: DerivedCategory }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2.5">
          <PixelIcon
            name={category.icon}
            size={20}
            className={hasEvidence ? "text-[var(--phos)]" : "text-faint"}
          />
        ...
        <span
          className={cn(
            "font-display shrink-0 rounded-full border px-2.5 py-1 text-[11px] font-bold tabular-nums",
            hasEvidence
              ? "border-[var(--phos)]/30 bg-[var(--phos)]/10 text-[var(--phos)]"
      ...
      >
        Retos para subir <PixelIcon name="arrowRight" size={12} />
      </Link>
    </div>
  );
}
