...
export const evaluate = action({
  ...
  handler: async (
    ctx,
    args,
  ): Promise<{ submissionId: Id<"submissions">; totalScore: number }> => {
    // TODO(build real): reemplazar el mock por una llamada ESTÁTICA a Claude
    // (Anthropic API) que lea repoUrl/demoUrl SIN ejecutar código y devuelva
    // fit/quality/architecture/security + strengths/issues. Requiere la env
    // ANTHROPIC_API_KEY en el deployment de Convex — NUNCA hardcodear la key.
    // Los efectos externos viven aquí (action); la persistencia va por
    // internalMutation (regla 6).
